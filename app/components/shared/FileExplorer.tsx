...
export default function FileExplorer({ data }: IFileExplorer) {
            ...
            >
                <div className=" relative">
                    ...
                    <motion.div
                        ...
                        className={`absolute border-l-2 border-b-2 bottom-0   -translate-x-full rounded-bl-xl mb-2 pointer-events-none w-0 h-0  ${index === 0 && level === 0 ? "border-transparent" : "border-white"
                            }`}
                    ></motion.div>
                </div>
    ...
    return (
        <div>{renderTree(newData, 0, 0)}</div>
    );
};
